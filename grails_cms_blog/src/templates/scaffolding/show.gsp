			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<div class="fieldcontain">
					...
					<%  } else if (p.oneToMany || p.manyToMany) { %>
						...
						<span class="property-value" aria-labelledby="${p.name}-label"><g:link controller="admin/${p.referencedDomainClass?.propertyName}" action="show" id="\${${p.name[0]}.id}">\${${p.name[0]}?.encodeAsHTML()}</g:link></span>
						</g:each>
					<%  } else if (p.manyToOne || p.oneToOne) { %>
						<span class="property-value" aria-labelledby="${p.name}-label"><g:link controller="admin/${p.referencedDomainClass?.propertyName}" action="show" id="\${${propertyName}?.${p.name}?.id}">\${${propertyName}?.${p.name}?.encodeAsHTML()}</g:link></span>
